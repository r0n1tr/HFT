...
def create_add_order(print_structure=False):
    ...
    price_bytes = struct.pack('>I', int(price))  # 4 bytes
...
def write_orders_to_file(filename="orders.txt"):
    with open(filename, "w") as f:
        for _ in range(10):
            order_type = random.choice(order_types)
            if order_type == 'ADD':
                hex_message = create_add_order()
            elif order_type == 'EXECUTE':
                hex_message = create_execute_order()
            elif order_type == 'CANCEL':
                hex_message = create_cancel_order()

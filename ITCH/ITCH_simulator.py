...
def create_add_order(print_structure=False, print_plain=False):
    ...
    if print_structure:
        if print_plain:
            # Print the order structure without hex conversion
            print(f"ADD Order:")
            print(f"  Order Type: ADD")
            ...
            # Print the order structure with hex conversion
            print(f"ADD Order:")
...
def create_execute_order(print_structure=False, print_plain=False):
    if not order_book:
        return None
    timestamp = generate_timestamp()
...
def create_cancel_order(print_structure=False, print_plain=False):
    if not order_book:
        return None
    timestamp = generate_timestamp()
    order_id = random.choice(list(order_book.keys()))
...
def generate_itch_stream(num_messages=100, print_structure=False, print_plain=False):
    for _ in range(num_messages):
        order_type = random.choice(order_types)
        if order_type == 'ADD':
            create_add_order(print_structure, print_plain)
        elif order_type == 'EXECUTE':
            create_execute_order(print_structure, print_plain)
        elif order_type == 'CANCEL':
            create_cancel_order(print_structure, print_plain)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--print_plain', action='store_true', help="Print the structure of the orders without hex conversion.")
    args = parser.parse_args()
    
    generate_itch_stream(100, args.print_structure, args.print_plain)
